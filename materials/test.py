...
class CourseViewSetTestCase(APITestCase):
    ...
    def test_post(self):
        # Testing POST-request API
        url = r"http://127.0.0.1:8000/api/courses/"
        data = {"title": "New test course", "description": "test course description"}
        response = self.client.post(url, data, format="json")
        dict_data = response.json()
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_delete_dpermission_denied(self):
        user = CustomUser.objects.create_user(
            ...
            username="testerroruser",
        )
        course = Course.objects.create(
            title="Test error course", description="test error description", owner=user
        )
...
class SubscriptionCreateDestroyAPIViewTestCase(APITestCase):
    ...
    def test_post(self):
        # Testing POST-request API
        url = r"http://127.0.0.1:8000/api/subscriptions/create/"
